  ...
  return (
    ...
    >
      ...
      <header className="space-y-2">
        <h1 className="flex text-3xl md:text-4xl font-extrabold tracking-tight text-slate-900 justify-center">
          {name || "Your Name"}
        </h1>
        <h3 className="flex text-xl md:text-md font-medium tracking-tight text-slate-800 justify-center">{job ? <span>{job}</span> : null}</h3>

        {/* Contact line (centered) */}
        <div className="text-sm text-slate-600 flex flex-wrap gap-x-3 gap-y-1 justify-center">
      ...
      {languages?.length > 0 &&
        renderSection(
          "LANGUAGES",
          themeColor,
          <ul className="list-disc pl-5 text-[13px] text-slate-800 space-y-1 text-left">
